...
const LogsPage = () => {
  ...
  const columns = [
    {
      ...
      render: (text: any, record: LogsReponse, index: number) =>
        index + 1 + (filter.page - 1) * filter.itemsPerPage,
    },
    {
      title: "URL",
      dataIndex: "url",
    ...
    },
    {
      ...
      key: "logLevel",
      align: "center" as const,
      render: (text: string) => renderLogLevel(text),
    },
    {
      title: "Time (ms)",
    ...
    },
    {
      ...
      render: (text: string, record: LogsReponse) => (
        <Button onClick={() => handleShowIPData(record.ip)}>IP Data</Button>
      ),
    },
  ];
  ...
  const getChartData = async () => {
    try {
      const logsResponse = await logsUseCase.getChartData(
